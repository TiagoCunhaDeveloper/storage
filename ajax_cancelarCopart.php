<?php
	...
	$sql="DELETE FROM `tb_compartilhados_interno` WHERE `tb_compartilhados_interno`.`id_compartilhados_interno` = $id";
	$DelCompart = $conex -> prepare($sql);
	$DelCompart -> execute();
	$sql1 = "SELECT * FROM tb_compartilhados_interno INNER JOIN tb_usuario ON tb_compartilhados_interno.receptor_interno=tb_usuario.id_usuario WHERE caminho_compartilhado_interno='$caminho' AND fk_usuario= $id_usuario";
	$recuperaCompart = $conex -> prepare($sql1);
	$recuperaCompart -> execute();
	echo '<br>
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered ">
                                    ...
                                    <tbody>';
		foreach($recuperaCompart as $f_c){
